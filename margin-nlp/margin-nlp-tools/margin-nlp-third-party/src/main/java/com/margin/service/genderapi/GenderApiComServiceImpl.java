...
@Service
public class GenderApiComServiceImpl extends AbstractGenderService implements GenderThirdPartyService {
    @Override
    public NLPDataResponse detectGender(NLPRequest request) {
        ...
        final String urlString = sourceCountry == null ?
                String.format("{url}", name, apiKey) :
                String.format("{url}" + "{localeParameter}", name, apiKey, sourceCountry);
